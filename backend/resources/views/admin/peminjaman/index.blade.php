            ...
            <tbody class="text-gray-700 text-sm">
                @forelse($peminjamans as $peminjaman)
                    <tr class="hover:bg-gray-50 transition align-top">
                        ...
                        <!-- Peminjam (FIX: Ambil nama dari relasi user) -->
                        <td class="py-3 px-4 border-b font-medium text-gray-900">
                            {{ $peminjaman->user->name ?? $peminjaman->user->nama ?? $peminjaman->nama_peminjam ?? 'Nama Tidak Ada' }}
                        </td>

                        <!-- Alat (FIX: Panggil detailPinjam tanpa 's' & ambil nama via relasi alat) -->
                        <td class="py-3 px-4 border-b">
                            <ul class="list-disc list-inside space-y-1">
                                ...
                                @if($details)
                                    ...
                                    @endforeach
                                @else
                                    <li class="text-xs text-gray-400">Tidak ada item</li>
                                @endif
                            </ul>
                        </td>
                        ...
                        <td class="py-3 px-4 border-b text-xs text-gray-600">
                            <span class="block">
                                Pinjam: {{ \Carbon\Carbon::parse($peminjaman->tgl_pinjam)->format('Y-m-d') }}
                            </span>
                            <span class="block font-semibold">
                                Rencana: {{ \Carbon\Carbon::parse($peminjaman->tgl_kembali_plan)->format('Y-m-d') }}
                            </span>
                        </td>
