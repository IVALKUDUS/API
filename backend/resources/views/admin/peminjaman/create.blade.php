...
<div class="max-w-2xl bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    ...
    <form action="{{ route('admin.peminjaman.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            @error('alat_id')
                <span class="text-red-500 text-xs block mb-2">
                    {{ $message }}
                </span>
            @enderror
            ...
            <button
                type="button"
                onclick="addRow()"
                class="mt-3 bg-gray-800 hover:bg-gray-900 text-white text-sm px-3 py-2 rounded-lg transition">
                + Tambah Alat Lain
            </button>
        </div>
...
<!-- Script Tambah/Hapus Baris Alat -->
<script>
    function addRow() {
        ...
        // Reset nilai select dan input pada baris baru
        const select = newRow.querySelector('select');
        select.value = '';

        const inputJumlah = newRow.querySelector('input[type="number"]');
        inputJumlah.value = '1';
    ...
    function removeRow(button) {
        ...
        if (rows.length > 1) {
            ...
            alert('Minimal harus ada 1 alat yang dimasukkan.');
        }
    }
</script>
