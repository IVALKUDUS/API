...
@section('header_title', 'Manajemen Kategori Alat')

@section('content')

@if(session('success'))
    <div class="mb-4 bg-emerald-50 border border-emerald-500 text-emerald-800 p-4 rounded-lg shadow-sm text-sm">
        {{ session('success') }}
    </div>
@endif

@if(session('error'))
    <div class="mb-4 bg-red-50 border border-red-500 text-red-800 p-4 rounded-lg shadow-sm text-sm">
        {{ session('error') }}
    </div>
@endif
    ...
    <div class="p-6 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
            ...
            <form action="{{ route('admin.kategori.index') }}" method="GET" class="flex w-full md:w-80">
                ...
                <button
                    type="submit"
                    class="bg-gray-800 hover:bg-gray-700 text-white px-4 py-2 text-sm font-semibold rounded-r-lg transition"
                >
                    Cari
                </button>
            ...
            @if(request('search'))
                <a
                    ...
                    class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 text-sm rounded-lg flex items-center transition"
                >
                    Reset
                </a>
            @endif
        ...
        <table class="w-full text-left border-collapse">
            ...
            <thead>
                <tr class="bg-gray-100 text-gray-600 text-sm uppercase tracking-wider">
                    ...
                    <th class="py-3 px-4 border-b w-40">
                        Aksi
                    </th>
            ...
            <tbody class="text-gray-700 text-sm">
                ...
                @forelse($kategoris as $indeks => $kategori)

                    <tr class="hover:bg-gray-50 transition">

                        <td class="py-3 px-4 border-b text-center">
                            {{ ($kategoris->firstItem() + $indeks) }}
                        </td>
